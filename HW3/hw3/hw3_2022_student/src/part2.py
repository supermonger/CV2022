...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            corners = np.array(corners)[0,...].reshape((4,2))
            ...
            ymax, ymin = np.max(corners[:,1]).astype(int), np.min(corners[:,1]).astype(int)
            xmax, xmin = np.max(corners[:,0]).astype(int), np.min(corners[:,0]).astype(int)
